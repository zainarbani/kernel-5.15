...
#define msrwarn(fmt, obj, subdev, frame, args...) \
	mwarn_common("[%d:%s][%s][F%d][WRN]%s:%d:", fmt "\n", obj->instance, obj->stm, subdev->name, frame->fcount, __func__, __LINE__, ##args)
...
#define minfo(fmt, obj, args...) \
	minfo_common("[%d:%s]", fmt, obj->instance, obj->stm, ##args)
...
#define mgrinfo(fmt, obj, group, frame, args...) \
	minfo_common("[%d:%s][%s][F%d]", fmt, obj->instance, obj->stm, group_id_name[group->id], frame->fcount, ##args)

#define mlvinfo(fmt, obj, vid, args...) \
	minfo_common("[%d:%s][%s]", fmt, obj->instance, obj->stm, vn_name[vid], ##args)

#define mcinfo(fmt, obj, args...) \
	minfo_common("[%d:%s][CSI%d][D%d]", fmt, obj->instance, obj->stm, obj->otf_info.csi_ch, obj->wdma_ch[0], ##args)
...
#define mdbgv_lvn(level, fmt, obj, video, args...) \
	mdbg_common((is_get_debug_param(IS_DEBUG_PARAM_LVN)) >= (level), "[%d][V%02d]", fmt, obj->instance, video->id, ##args)
...
#define mdbgd_sensor(fmt, this, args...) \
	mdbg_common(is_get_debug_param(IS_DEBUG_PARAM_DEVICE), "[%d:%s][SEN:D]", fmt, this->instance, this->stm, ##args)

#define mdbgd_front(fmt, this, args...) \
	mdbg_common(is_get_debug_param(IS_DEBUG_PARAM_DEVICE), "[%d:%s][FRT:D]", fmt, this->instance, this->stm, ##args)

#define mdbgd_back(fmt, this, args...) \
	mdbg_common(is_get_debug_param(IS_DEBUG_PARAM_DEVICE), "[%d:%s][BAK:D]", fmt, this->instance, this->stm, ##args)

#define mdbgd_ischain(fmt, this, args...) \
	mdbg_common(is_get_debug_param(IS_DEBUG_PARAM_DEVICE), "[%d:%s][ISC:D]", fmt, this->instance, this->stm, ##args)

#define mdbgd_group(fmt, this, args...) \
	mdbg_common(is_get_debug_param(IS_DEBUG_PARAM_DEVICE), "[%d:%s][%s:D]", fmt, this->instance, this->stm, group_id_name[group->id], ##args)
...
#define mvdbgs(level, fmt, obj, queue, args...) \
	mdbg_common((is_get_debug_param(IS_DEBUG_PARAM_STREAM)) >= (level), "[%d:%s][%s:S]", fmt, obj->instance, obj->stm, (queue)->name, ##args)

#define mgrdbgs(level, fmt, obj, group, frame, args...) \
	mdbg_common((is_get_debug_param(IS_DEBUG_PARAM_STREAM)) >= (level), "[%d:%s][%s:S][F%d]", fmt, obj->instance, obj->stm, group_id_name[group->id], frame->fcount, ##args)

#define msdbgs(level, fmt, obj, subdev, args...) \
	mdbg_common((is_get_debug_param(IS_DEBUG_PARAM_STREAM)) >= (level), "[%d:%s][%s:S]", fmt, obj->instance, obj->stm, subdev->name, ##args)

#define msrdbgs(level, fmt, obj, subdev, frame, args...) \
	mdbg_common((is_get_debug_param(IS_DEBUG_PARAM_STREAM)) >= (level), "[%d:%s][%s:S][F%d]", fmt, obj->instance, obj->stm, subdev->name, frame->fcount, ##args)

#define mdbgs_ischain(level, fmt, obj, args...) \
	mdbg_common((is_get_debug_param(IS_DEBUG_PARAM_STREAM)) >= (level), "[%d:%s][ISC:S]", fmt, obj->instance, obj->stm, ##args)

#define mdbgs_sensor(level, fmt, obj, args...) \
	mdbg_common((is_get_debug_param(IS_DEBUG_PARAM_STREAM)) >= (level), "[%d:%s][SEN:S]", fmt, obj->instance, obj->stm, ##args)

#define msdbgs_hw(level, fmt, instance, obj, args...) \
	dbg_common((is_get_debug_param(IS_DEBUG_PARAM_STREAM)) >= (level), "[%d:%s][HW:%s:S]", fmt, instance, obj->stm[instance], obj->name, ##args)

#define mlvdbgs(level, fmt, obj, vid, args...) \
	mdbg_common((is_get_debug_param(IS_DEBUG_PARAM_STREAM)) >= (level), "[%d:%s][%s]", fmt, obj->instance, stm(obj->instance), vn_name[vid], ##args)
/*
 * =================================================================================================
 * LOG - PROBE
 * =================================================================================================
...
#define dbg_isr(level, fmt, obj, args...)		\
	dbg_common((is_get_debug_param(IS_DEBUG_PARAM_IRQ)) >= (level), "[%d:%s][%s]", fmt, obj->instance, obj->stm, obj->name, ##args)
...
#define mdbg_hw(level, fmt, instance, args...) \
	dbg_common((is_get_debug_param(IS_DEBUG_PARAM_HW)) >= (level), "[%d:%s][HW]", fmt, instance, stm(instance), ##args)
#define sdbg_hw(level, fmt, obj, args...) \
	dbg_common((is_get_debug_param(IS_DEBUG_PARAM_HW)) >= (level), "[HW:%s]", fmt, obj->name, ##args)
#define msdbg_hw(level, fmt, instance, obj, args...) \
	dbg_common((is_get_debug_param(IS_DEBUG_PARAM_HW)) >= (level), "[%d:%s][HW:%s]", fmt, instance, stm(instance), obj->name, ##args)
...
#define sinfo_hw(fmt, obj, args...) \
	info_common("[HW:%s]", fmt, obj->name, ##args)
...
#define mswarn_hw(fmt, instance, obj, args...) \
	warn_common("[%d:%s][HW:%s][WRN]%d:", fmt "\n", instance, stm(instance), obj->name, __LINE__, ##args)
...
#define mserr_hw(fmt, instance, obj, args...) \
	err_common("[%d:%s][HW:%s][ERR]%s:%d:", fmt "\n", instance, obj->stm[instance], obj->name, __func__, __LINE__, ##args)
...
#define mdbg_adt(level, fmt, instance, args...) \
	dbg_common((is_get_debug_param(IS_DEBUG_PARAM_HW)) >= (level), "[%d:%s][ADT]", fmt, instance, stm(instance),  ##args)
