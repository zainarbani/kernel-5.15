...
static inline void is_pdp_frame_end_inline(struct is_hw_ip *hw_ip)
{
	...
	if (test_and_clear_bit(HW_SUSPEND, &hw_ip->state)) {
		CALL_HWIP_OPS(hw_ip, sensor_stop, instance);
...
static void is_hw_pdp_g_input(struct pablo_camif_otf_info *otf_info, struct is_pdp *pdp)
{
	...
	} else if (sensor_cfg->set_vc) {
		pdp->img_vc = sensor_cfg->img_vc[otf_out_id];
		pdp->af_vc = sensor_cfg->af_vc[otf_out_id];
	} else {
		pdp->img_vc = (otf_out_id == CAMIF_OTF_OUT_SINGLE) ? 0 : 1;
		pdp->af_vc = (otf_out_id == CAMIF_OTF_OUT_SINGLE) ? 2 : 3;
	}
}
...
static int is_hw_pdp_init_config(struct is_hw_ip *hw_ip, u32 instance, struct is_frame *frame)
{
	...
	is_hw_pdp_g_input(&csi->otf_info, pdp);
